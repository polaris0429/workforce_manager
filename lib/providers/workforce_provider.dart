...
class WorkforceProvider with ChangeNotifier {
  ...
  Map<String, BackupPathStatus> get backupStatusMap =>
      _backupSvc.statusMap;
  Stream<Map<String, BackupPathStatus>> get backupStatusStream =>
      _backupSvc.statusStream;
  ...
  void _scheduleBackup() {
    ...
    _debounceTimer = Timer(_debounce, () {
      unawaited(
        _backupSvc.backupAll(backupPaths: _backupPaths).then((_) {
          _refreshPending();
        }),
      );
    });
  ...
  Future<void> addBackupPath(String path) async {
    ...
    notifyListeners();
    unawaited(
      _backupSvc.backupAll(backupPaths: [path]).then((_) => _refreshPending()),
    );
  }
  ...
  Future<void> _backupImage(String localPath) async {
    ...
    await _backupSvc.backupImage(
        localImagePath: localPath, backupPaths: _backupPaths);
  }
  ...
  Future<void> addWorker({
    ...
    String gender = '', String residentNumber = '',
    required String phone,
    String address = '', String bankName = '', String bankAccount = '',
    String career = '', String notes = '',
    File? idPhotoFront, File? idPhotoBack,
  }) async {
    String? fp, bp;
    if (idPhotoFront != null) {
      fp = await ImageHelper.saveImageLocally(idPhotoFront, workerName: name);
      await _backupImage(fp);
    }
    if (idPhotoBack != null) {
      bp = await ImageHelper.saveImageLocally(idPhotoBack, workerName: name);
      await _backupImage(bp);
    }
    final w = Worker(
      id: _generateId(), name: name, gender: gender,
      residentNumber: residentNumber, phone: phone, address: address,
    ...
    _workers = await _dbSvc.getAllWorkers(); // 이름순 정렬 유지
    notifyListeners();
    _scheduleBackup();
  }
  ...
  Future<void> updateWorker({
    ...
    File? newFrontImage, File? newBackImage,
  }) async {
    final idx = _workers.indexWhere((w) => w.id == id);
    if (idx == -1) return;
    final old = _workers[idx];

    final wName = (data['name'] as String?)?.isNotEmpty == true
        ? data['name'] as String
        : old.name;

    if (newFrontImage != null) {
      final path = await ImageHelper.saveImageLocally(newFrontImage, workerName: wName);
      data['id_photo_path'] = path;
      await _backupImage(path);
    }
    if (newBackImage != null) {
      final path = await ImageHelper.saveImageLocally(newBackImage, workerName: wName);
      data['id_photo_back_path'] = path;
      await _backupImage(path);
    }
  ...
  Future<void> toggleBlacklist(String id, bool currentStatus, String? reason) async {
    ...
    final old = _workers[idx];
    final updated = Worker(
      id: old.id, name: old.name, gender: old.gender,
      residentNumber: old.residentNumber, phone: old.phone,
      address: old.address, bankName: old.bankName,
      bankAccount: old.bankAccount, career: old.career, notes: old.notes,
      idPhotoPath: old.idPhotoPath, idPhotoBackPath: old.idPhotoBackPath,
      isBlacklisted:   !currentStatus,
      blacklistReason: !currentStatus ? reason : null,
      createdAt: old.createdAt,
    );
    await _dbSvc.updateWorker(updated);
  ...
  Future<void> addAttendance({
    ...
    String? fp, bp;
    if (idPhotoFront != null) {
      fp = await ImageHelper.saveImageLocally(idPhotoFront, workerName: workerName);
      await _backupImage(fp);
    }
    if (idPhotoBack != null) {
      bp = await ImageHelper.saveImageLocally(idPhotoBack, workerName: workerName);
      await _backupImage(bp);
    }
    ...
    final att  = Attendance(
      ...
      workerResidentNumber: workerResidentNumber,
      workerPhone:          workerPhone,
      workerAddress:        workerAddress,
      workerBankName:       workerBankName,
      workerBankAccount:    workerBankAccount,
      workerCareer:         workerCareer,
      clientId:             clientId,
      ...
      commission:           comm,
      netWage:              isPostpaid ? dailyWage : (dailyWage - comm),
      notes:                notes,
      idPhotoPath:          fp,
      idPhotoBackPath:      bp,
      isPostpaid:           isPostpaid,
      isSettled:            !isPostpaid,
      createdAt:            DateTime.now(),
    );
  ...
  Future<void> updateAttendance({
    ...
    // 일당/수수료율/지급방식이 모두 있으면 재계산
    if (data.containsKey('daily_wage') &&
        data.containsKey('commission_rate') &&
        data.containsKey('is_postpaid')) {
      final double dw   = (data['daily_wage'] as num).toDouble();
      final double rate = (data['commission_rate'] as num).toDouble();
      final bool   pp   = data['is_postpaid'] as bool;
      final double comm = dw * (rate / 100);
      data['commission'] = comm;
      data['net_wage']   = pp ? dw : (dw - comm);
  ...
  Future<void> settleCommission(String id) async {
    ...
    final idx = _attendanceList.indexWhere((a) => a.id == id);
    if (idx != -1) {
      _attendanceList[idx].isSettled = true;
      notifyListeners();
      _scheduleBackup();
    }
  }
  ...
  // 통계
  // =========================================================
  List<Attendance> get unpaidCommissions =>
      _attendanceList.where((a) => a.isPostpaid && !a.isSettled).toList();
  double get totalUnpaidAmount =>
      unpaidCommissions.fold(0.0, (s, a) => s + a.commission);

  double get todayIncome {
    final today = DateTime.now();
    return _attendanceList
        .where((a) => _isSameDay(a.workDate, today) && a.isSettled)
        .fold(0.0, (s, a) => s + a.commission);
  }

  int get todayWorkersCount {
    final today = DateTime.now();
    return _attendanceList.where((a) => _isSameDay(a.workDate, today)).length;
  }
  ...
  double get weeklyIncome {
    ...
    return _attendanceList
        .where((a) => !a.workDate.isBefore(start) && a.isSettled)
        .fold(0.0, (s, a) => s + a.commission);
  }
  ...
  double get monthlyIncome {
    ...
    return _attendanceList
        .where((a) =>
            a.workDate.year == now.year &&
            a.workDate.month == now.month &&
            a.isSettled)
        .fold(0.0, (s, a) => s + a.commission);
  }
  ...
  // 엑셀 내보내기
  // =========================================================
  CellStyle _makeColorStyle(String hex) => CellStyle(
  ...
  void _writeAttendanceSheet(Sheet sheet, List<Attendance> records) {
    ...
    final ss = _makeColorStyle('#92D050');
    final us = _makeColorStyle('#FF0000');
    final ds = _makeColorStyle('#87CEEB');
    final cs = _makeColorStyle('#92D050');
    final ts = CellStyle(
        backgroundColorHex: ExcelColor.fromHexString('#92D050'),
        fontColorHex: ExcelColor.fromHexString('#000000'),
        bold: true);
    ...
    for (int i = 0; i < records.length; i++) {
      ...
      sheet.cell(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: row)).value =
          TextCellValue(att.workDate.toIso8601String().split('T')[0]);
      sheet.cell(CellIndex.indexByColumnRow(columnIndex: 1, rowIndex: row)).value =
          TextCellValue(att.workerName);
      sheet.cell(CellIndex.indexByColumnRow(columnIndex: 2, rowIndex: row)).value =
          TextCellValue(att.clientName);

      final dc = sheet.cell(CellIndex.indexByColumnRow(columnIndex: 3, rowIndex: row));
      dc.value = IntCellValue(att.dailyWage.toInt()); dc.cellStyle = ds;

      final cc = sheet.cell(CellIndex.indexByColumnRow(columnIndex: 4, rowIndex: row));
      cc.value = IntCellValue(att.commission.toInt()); cc.cellStyle = cs;

      sheet.cell(CellIndex.indexByColumnRow(columnIndex: 5, rowIndex: row)).value =
          TextCellValue(att.isPostpaid ? '후불' : '선불');

      final sc = sheet.cell(CellIndex.indexByColumnRow(columnIndex: 6, rowIndex: row));
      sc.value     = TextCellValue(att.isSettled ? '완료' : '미수');
      sc.cellStyle = att.isSettled ? ss : us;

      if (i == 0) {
        final tc = sheet.cell(CellIndex.indexByColumnRow(columnIndex: 7, rowIndex: row));
        tc.value = IntCellValue(total.toInt()); tc.cellStyle = ts;
      }
    }
  }
  ...
  List<Attendance> _filterByPeriod(ExcelPeriodType period) {
    final now = DateTime.now();
    switch (period) {
      case ExcelPeriodType.today:
        return _attendanceList.where((a) => _isSameDay(a.workDate, now)).toList();
      case ExcelPeriodType.week:
        final start = DateTime(now.year, now.month, now.day - (now.weekday - 1));
        return _attendanceList.where((a) => !a.workDate.isBefore(start)).toList();
      case ExcelPeriodType.month:
        return _attendanceList
            .where((a) => a.workDate.year == now.year && a.workDate.month == now.month)
            .toList();
      case ExcelPeriodType.all:
        return List.from(_attendanceList);
    }
  }
  ...
  String _periodLabel(ExcelPeriodType period) {
    ...
    switch (period) {
      case ExcelPeriodType.today:
        return '일별_${now.year}${d(now.month)}${d(now.day)}';
      case ExcelPeriodType.week:
        final mon = now.subtract(Duration(days: now.weekday - 1));
        return '주별_${mon.year}${d(mon.month)}${d(mon.day)}';
      case ExcelPeriodType.month:
        return '월별_${now.year}${d(now.month)}';
      case ExcelPeriodType.all:
        return '전체_${now.year}${d(now.month)}${d(now.day)}';
    }
  }
  ...
  Future<String> exportToExcel() async =>
      exportToExcelByPeriod(ExcelPeriodType.all);
}
