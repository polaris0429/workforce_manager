...
class ImageHelper {
  ...
  static Future<String> _getWorkerImageDir(String workerName) async {
    final dataDir = await StorageLocation.dataDirectory;
    ...
    final dirPath = p.join(dataDir, _imageFolder, safeName);
    final dir = Directory(dirPath);
    if (!await dir.exists()) await dir.create(recursive: true);
    return dirPath;
  }

  /// 이미 저장된 사진은 기존 경로를 참조하고, 외부 사진만 복사합니다.
  ...
  static Future<String> saveImageLocally(
    ...
    final dataDir = await StorageLocation.dataDirectory;
    final imageRoot = p.normalize(p.absolute(
        p.join(dataDir, _imageFolder)));
    final sourcePath = p.normalize(p.absolute(sourceFile.path));
    if (p.isWithin(imageRoot, sourcePath)) {
      if (!await sourceFile.exists()) {
        throw FileSystemException('저장된 사진을 찾을 수 없습니다.', sourcePath);
      }
      return sourcePath;
    }
